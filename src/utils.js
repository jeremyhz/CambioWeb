...
Cambio.fadeOutIn = function(passedCallback, context) {
...
Cambio.fadeOutScene = function(sceneName, context) {
  context.cameras.main.fadeOut(250);
  context.time.addEvent({
      delay: 250,
      callback: function() {
...
Cambio.Lang = {
  ...
  updateLanguage: function(lang) {
    ...
    var qs = Cambio.Lang.parseQueryString(query);
    if(qs && qs['lang']) {
      console.log('LANG: '+qs['lang']);
      Cambio.Lang.current = qs['lang'];
    } else {
      if(lang) {
        Cambio.Lang.current = lang;
      }
      else {
        Cambio.Lang.current = navigator.language;
      }
    }
    if(Cambio.Lang.options.indexOf(Cambio.Lang.current) == -1) {
      Cambio.Lang.current = 'en';
    }
  },
  text: {
    'en': {
